...
import {
  ...
  Scale, Barcode, Camera, Loader, Calendar, Star, BookMarked, ChevronDown, Layers, Bookmark, Volume2, VolumeX,
} from "lucide-react";
...
/* ============================================================
   ...
   ============================================================ */
// available Today-screen metrics (value computed per-day; goal from settings[goalKey])
const TODAY_METRICS = {
  energy:   { label: "Energy",   unit: "cal",  c: [C.energy0, C.energy1], goalKey: "goalCalories", Icon: Flame },
  protein:  { label: "Protein",  unit: "g",    c: [C.protein0, C.protein1], goalKey: "goalProtein", Icon: Apple },
  carbs:    { label: "Carbs",    unit: "g",    c: ["#C77F00", C.carb], goalKey: "goalCarbs", Icon: Flame },
  fat:      { label: "Fat",      unit: "g",    c: ["#C7A400", C.fat], goalKey: "goalFat", Icon: Flame },
  sets:     { label: "Training", unit: "sets", c: [C.train0, C.train1], goalKey: "goalSets", Icon: Dumbbell },
  time:     { label: "Time",     unit: "min",  c: [C.train0, C.train1], goalKey: "goalTrainMin", Icon: Clock },
  burned:   { label: "Burned",   unit: "cal",  c: [C.energy0, C.energy1], goalKey: "goalBurn", Icon: Activity },
  volume:   { label: "Volume",   unit: "",     c: ["#3A7BD5", "#00D4FF"], goalKey: "goalVolume", Icon: BarChart3 },
  distance: { label: "Distance", unit: "mi",   c: [C.train0, C.train1], goalKey: "goalDistance", Icon: Activity },
  workouts: { label: "Workouts", unit: "",     c: [C.train0, C.train1], goalKey: "goalWorkouts", Icon: Dumbbell },
};
const TODAY_METRIC_ORDER = ["energy", "protein", "carbs", "fat", "sets", "time", "burned", "volume", "distance", "workouts"];
...
function LogoMark({ size = 36 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 100 100">
      ...
      <rect x="24" y="50" width="13" height="28" rx="6.5" fill="url(#lg-e)" />
      <rect x="43.5" y="36" width="13" height="42" rx="6.5" fill="url(#lg-p)" />
      <rect x="63" y="22" width="13" height="56" rx="6.5" fill="url(#lg-t)" />
    </svg>
  );
}
        ...
        {routines.map((rt) => (
          <Card key={rt.id} className="p-4">
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-1">
                <button onClick={() => onEditRoutine(rt)} className="p-1.5"><Pencil size={15} color={C.faint} /></button>
                <button onClick={() => onDeleteRoutine(rt.id)} className="p-1.5"><Trash2 size={15} color={C.faint} /></button>
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center justify-between pt-1">
        ...
        <div className="flex items-center gap-1.5 rounded-full px-3 py-1" style={{ background: C.card }}>
          <Clock size={14} color={C.train1} /><span className="font-mono text-sm font-bold" style={{ color: C.train1 }}>{mmss(elapsed)}</span>
        </div>
      </div>
...
/* ============================================================
   ...
   ============================================================ */
function FoodView({ date, setDate, dayFood, nutri, settings, onAdd, onRemove, onOpenLibrary, onSaveAsMeal }) {
      ...
      {MEALS.map((meal) => {
        ...
        return (
          <Card key={meal} className="p-4">
            ...
            {items.map((it, idx) => (
              <div key={idx} className="flex items-center justify-between border-t py-2" style={{ borderColor: C.line }}>
                <div className="flex-1">
    ...
    <Sheet title="My Library" onClose={onClose} accent={C.carb}>
      ...
      {tab === "foods" ? (
        <div className="mt-3 pb-2">
          ...
          {customFoods.map((f, i) => (
            <div key={(f.barcode || f.name) + i} className="flex items-center gap-1 border-t py-2.5" style={{ borderColor: C.line }}>
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-semibold">{f.name}</p>
                <p className="text-xs" style={{ color: C.sub }}>{f.serving} · {f.cal} cal · P{f.p} C{f.c} F{f.f}</p>
              </div>
              <button onClick={() => onEditFood(f)} className="p-1.5"><Pencil size={15} color={C.faint} /></button>
              <button onClick={() => onDeleteFood(f)} className="p-1.5"><Trash2 size={15} color={C.faint} /></button>
            </div>
          ))}
        </div>
      ) : (
        <div className="mt-3 pb-2">
          ...
          {meals.map((m) => {
            const t = mealTotals(m.items);
            return (
              <div key={m.id} className="flex items-center gap-1 border-t py-2.5" style={{ borderColor: C.line }}>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-semibold">{m.name}</p>
                  <p className="text-xs" style={{ color: C.sub }}>{m.items.length} items · {round(t.cal)} cal · P{round(t.p)} C{round(t.c)} F{round(t.f)}</p>
                </div>
                <button onClick={() => onEditMeal(m)} className="p-1.5"><Pencil size={15} color={C.faint} /></button>
                <button onClick={() => onDeleteMeal(m.id)} className="p-1.5"><Trash2 size={15} color={C.faint} /></button>
              </div>
            );
          })}
        </div>
